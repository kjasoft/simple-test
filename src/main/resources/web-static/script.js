...
m.controller('MyContr', function MyContr($scope, Server) {
	...
	$scope.start = function() {
		var fio = $scope.data.fio;
		if(!fio) {
			alert('Введiть Ваше ПIБ');
			return;
		}
		Server.get({oper:'saveFio', fio: fio}, function(data) {
			startApply();
		});
	}
	...
	function checkResults(answers, showResults) {
		Server.save({oper:'checkResults'}, {questions: answers, fio: $scope.data.fio}, 
				function(data) 
		{
			/*{
				right: 7,
